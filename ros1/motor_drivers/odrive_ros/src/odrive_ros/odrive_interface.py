...
class ODriveInterfaceAPI(object):
    ...
    def connect(self, port=None, right_axis=0, timeout=30, whichser="def"):
        if self.driver:
        ...
        try:
            ...
            #self.driver = odrive.find_any(serial_number="207A3892304E", timeout=timeout, logger=self.logger)
            #self.driver = odrive.find_any(path="usb", serial_number="207A3892304E", timeout=timeout, logger=self.logger)
            #odrives = odrive.find_any(find_multiple=2)
            #self.driver2 = odrives[0]
            #self.driver = odrives[1]

            # doesn't work
            #self.driver = odrive.find_any("serial:/dev/serial/by-id/usb-ODrive_Robotics_ODrive_3.6_CDC_Interface_207A3892304E-if00")
            # self.driver = odrive.find_any(serial_number="35752257007694", timeout=timeout, logger=self.logger)
            # self.driver = odrive.find_any(path="usb", serial_number="35752257007694", timeout=timeout, logger=self.logger)

            # If the serial is less than 4 characters (default abc) just open any odrive
            if len(whichser) < 4:
              self.driver = odrive.find_any(timeout=timeout, logger=self.logger)
            else:
              self.driver = odrive.find_any(path="usb", serial_number=whichser, timeout=timeout, logger=self.logger)
            self.axes = (self.driver.axis0, self.driver.axis1)
        except:
    ...
    def drive(self, left_motor_val, right_motor_val):
        if not self.driver:
            ...
            return
        #try:
        # For hoverboard bots need different right/left value, for rock crawler same, but opposite
    ...
    def set_high_gains(self):
        self.left_axis.controller.config.vel_gain  = 7.0
        self.right_axis.controller.config.vel_gain = 7.0
        self.left_axis.controller.config.vel_integrator_gain  = 9.0
        self.right_axis.controller.config.vel_integrator_gain = 9.0

    def set_low_gains(self):
        self.left_axis.controller.config.vel_gain  = 0.7
        self.right_axis.controller.config.vel_gain = 0.7
        self.left_axis.controller.config.vel_integrator_gain  = 1.4
        self.right_axis.controller.config.vel_integrator_gain = 1.4
    ...
    def estop_val(self):          return self.driver.get_adc_voltage(5)       if self.driver else 0.   # analog voltage value - 1.5 (go) or 0.0 (estop) typically
    #def estop_val(self):           return 1.44 # if needed for testing

    # TODO check these match the right motors, but it doesn't matter for now
    def left_temperature(self):   return self.left_axis.fet_thermistor.temperature  if self.left_axis  else 0.
    def right_temperature(self):  return self.right_axis.fet_thermistor.temperature if self.right_axis else 0.
