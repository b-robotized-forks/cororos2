...
class ODriveNode(object):
    ...
    prerolling = False
    set_high_gains = 0
    set_low_gains = 1   # Set low gains by default to start
    ...
    def __init__(self):
        ...
        self.serial_number     = get_param('~serial_number', "abc")
        ...
        self.estop_val_publisher = rospy.Publisher('estop_val', Float64, queue_size=2)
        self.vbus_val_publisher = rospy.Publisher('vbus_val', Float64, queue_size=2)
    ...
    def fast_timer(self, timer_event):
        ...
        if self.fast_timer_comms_active:
            try:
                ...
                if error_string:
                    ...
                    self.encoder_cpr = 1.0      # returns rotations rather than counts with new firmware
                    self.m_s_to_value = 1.0/self.tyre_circumference # calculated
                    ...
                    # estop_val - read switch from adc
                    self.estop_val = self.driver.estop_val()

                    # gains
                    if (self.set_high_gains == 1):
                        self.set_high_gains = 0
                        self.driver.set_high_gains()
                    if (self.set_low_gains == 1):
                        self.set_low_gains = 0
                        self.driver.set_low_gains()
        ...
        self.estop_val_publisher.publish(float(self.estop_val))
        self.vbus_val_publisher.publish(float(self.bus_voltage))
        try:
            ...
            if self.driver:
                ...
                if (time_now - self.last_cmd_vel_time).to_sec() > 1.0 and self.driver.engaged():
    ...
    def connect_driver(self, request):
        ...
        if not self.driver.connect(right_axis=self.axis_for_right, whichser=self.serial_number):
            self.driver = None
    ...
    def convert(self, forward, ccw):
        ...
        # special case for rock crawler
        if (ccw > 8900.0):
            left_linear_val  = -forward * self.m_s_to_value
            right_linear_val = -forward * self.m_s_to_value
    ...
    def cmd_vel_callback(self, msg):
        ...
        if (msg.linear.z > 0.5):
            self.set_high_gains = 1
        if (msg.linear.z < -0.5):
            self.set_low_gains = 1
    ...
    def pub_odometry(self, time_now):
        ...
        # causes error - comment out for now
        #if self.publish_raw_odom:
