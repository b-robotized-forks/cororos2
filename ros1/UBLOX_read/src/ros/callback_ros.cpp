...
// Callback function for RTCM from custom stream
void UBLOX_ROS::cb_rtcm(const shared_messages::Command &msg) {
	...
	encrypt((uint8_t*)&msg.rtcm1[0], msg.rtcm1.size(), &encrypted_buf, &encrypted_size);
	ublox_->udp_read_cb(encrypted_buf, encrypted_size);
    }
    if (msg.rtcm2.size() > 0) {
	size_t encrypted_size;
	const uint8_t *encrypted_buf;
	encrypt((uint8_t*)&msg.rtcm2[0], msg.rtcm2.size(), &encrypted_buf, &encrypted_size);
	ublox_->udp_read_cb(encrypted_buf, encrypted_size);
    }
}
