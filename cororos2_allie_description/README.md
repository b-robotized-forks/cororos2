...
Allie robot description package
...
2. Install the package by calling `colcon build --symlink-install --packages-select cororos2_allie_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.b-robotized.com) than instead of the previous three steps, use `cb cororos2_allie_description` command.
...
or
```
ros2 launch cororos2_allie_description view_allie.launch.py
```
